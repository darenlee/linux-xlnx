...
static int mw_stream_iio_buffer_preenable(struct iio_dev *indio_dev)
{
	...
	/* START: reset buffer and change data type  */
	
	// remove from managed list
	dev_dbg(&mwchan->dev, "Calling mw_stream_change_channel_datatype\n");

	status = mw_stream_change_channel_datatype(indio_dev); // change data type
	if(status)
	{
		dev_dbg(&mwchan->dev, "mw_stream_change_channel_datatype returned %d\n",status);
	}

 	status = iio_update_buffers(indio_dev,indio_dev->buffer,NULL);

	if(status)
		dev_dbg(&mwchan->dev, "iio_update_buffers returned %d. Buffer should be disabled..\n",status);

	//indio_dev->buffer->access->set_bytes_per_datum(indio_dev->buffer,2); //change to int16...
	//now reconfigure the buffer again...
	//dev_dbg(&mwchan->dev, "Start configuring buffer... \n");
	//status = devm_mw_stream_configure_buffer(indio_dev);
	//if (status){
	//	dev_dbg(&mwchan->dev, "devm_mw_stream_configure_buffer returned %d\n",status);
	//}
	//dev_dbg(&mwchan->dev, "Done configuring buffer... \n");
	
	/* DONE: reset buffer and change data type  */
...
static int devm_mw_stream_iio_register(struct iio_dev *indio_dev) {
    ...
    dev_dbg(&mwchan->dev, "Call devm_iio_device_register  \n");
